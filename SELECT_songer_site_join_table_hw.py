...
def sel_4():
    ...
    select_4 = connection.execute("""SELECT DISTINCT a.artist_name FROM artist_album aa
    JOIN album ab ON aa.id_album = ab.id_album
    JOIN artist a ON aa.id_artist = a.artist_id
...
def sel_8():
    ...
    select_8 = connection.execute("""SELECT  a.artist_name, MIN(t.track_duration) FROM artist a
    JOIN track t ON a.artist_id = t.artist_id
    GROUP BY a.artist_name;
    """).fetchall()

    for artists in select_8:
        print(f'--> "{artists[0]}" имеет наименьшую продолжительность - {artists[1]} секунд')
    print('\n')


def sel_9():
    print(str(f'название альбомов, содержащих наименьшее количество треков').upper())
    select_9 = connection.execute("""WITH track_counts AS
    (SELECT album_name, COUNT(track_id_is_album) as number_track FROM album a
    JOIN track t ON a.id_album = t.track_id_is_album
    GROUP BY album_name)
    SELECT album_name FROM track_counts
    WHERE number_track = (SELECT MIN(number_track) FROM track_counts);
    """).fetchall()
